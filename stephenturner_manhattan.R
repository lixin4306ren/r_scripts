...
manhattan = function(dataframe, colors=c("gray10", "gray50"), ymax="max", cex.x.axis=1, cex.x=0.5,limitchromosomes=1:40, suggestiveline=-log10(1e-5), genomewideline=-log10(5e-8), annotate=NULL, ...) {
    ...
    if (numchroms==1) {
        with(d, plot(pos, logp, ylim=c(0,ymax), ylab=expression(italic(pi)), xlab=paste("Chromosome",unique(d$CHR),"position"), ...))
    }	else {
        with(d, plot(pos, logp, ylim=c(0,ymax), ylab=expression(italic(pi)), xlab="Chromosome", xaxt="n", type="n", ...))
        axis(1, at=ticks, lab=unique(d$CHR), cex.axis=cex.x.axis)
        icol=1
        for (i in unique(d$CHR)) {
            with(d[d$CHR==i, ],points(pos, logp, cex=cex.x,col=colors[icol], ...))
            icol=icol+1
    	}
    }
    ...
    if (suggestiveline) abline(h=suggestiveline, col="blue",lty="dashed")
    if (genomewideline) abline(h=genomewideline, col="red",lty="dashed")
}
